...
async def create_invoice(
    ...
    payload = json.dumps(
        {"user_id": chat_id, "months": months, "timestamp": datetime.utcnow().isoformat()}
    )
    ...
    try:
        ...
        # Common misconfiguration: provider token is set but invalid for this bot/provider
        if "PAYMENT_PROVIDER_INVALID" in str(e):
            logger.error(
                "PAYMENT_PROVIDER_INVALID: check PROVIDER_TOKEN and BotFather -> Payments for this bot"
            )
            raise
...
async def handle_pre_checkout(query: PreCheckoutQuery) -> None:
    ...
    try:
        ...
        async with AsyncSessionLocal() as session:
            ...
            logger.info(
                f"Created pending payment record for user {user_id}, amount={query.total_amount}"
            )
