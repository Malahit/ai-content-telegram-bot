...
async def add_user(user: User) -> User:
    ...
    async with AsyncSessionLocal() as session:
        try:
            ...
            logger.exception(
                "IntegrityError during add_user but existing user not found after rollback"
            )
            raise
...
async def activate_subscription(telegram_id: int, months: int = 1) -> Optional[User]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        if user:
            ...
            await session.refresh(user)
            logger.info(
                f"Activated subscription for user {telegram_id}: {months} month(s), expires {user.subscription_end}"
            )
            return user
...
async def count_premium() -> int:
    ...
    """
    async with AsyncSessionLocal() as session:
        result = await session.execute(select(func.count(User.id)).where(User.is_premium.is_(True)))
        count = result.scalar()
        return count or 0
    ...
    async with AsyncSessionLocal() as session:
        ...
        try:
            ...
            if existing:
                ...
                return existing
            logger.exception(
                "IntegrityError during register_or_get_user but existing user not found after rollback"
            )
            raise


async def update_user_role(
    telegram_id: int, new_role: UserRole, admin_id: Optional[int] = None
) -> bool:
    """
...
async def update_user_status(
    telegram_id: int, new_status: UserStatus, admin_id: Optional[int] = None
) -> bool:
    """
...
async def _resolve_or_create_user_pk(session, telegram_id: int) -> Optional[int]:
    """Resolve internal users.id by telegram_id, optionally creating a stub user."""
    result = await session.execute(select(User).where(User.telegram_id == telegram_id))
    user = result.scalar_one_or_none()
    if user:
        return user.id

    # Create minimal stub user (avoid recursion to register_or_get_user which logs actions)
    user = User(
        telegram_id=telegram_id,
        username=None,
        first_name=None,
    ...
    try:
        ...
        await session.commit()
        await session.refresh(user)
        logger.warning(f"Created stub user record for telegram_id={telegram_id} to satisfy logs FK")
        return user.id
    except IntegrityError:
        await session.rollback()
        # Someone else inserted concurrently — try again
        result = await session.execute(select(User).where(User.telegram_id == telegram_id))
        user = result.scalar_one_or_none()
...
async def add_log(telegram_id: int, action: str) -> None:
    ...
    NOTE: logs.user_id is a FK to users.id (internal PK), so we must resolve it
    from telegram_id.
    ...
    async with AsyncSessionLocal() as session:
        try:
            user_pk = await _resolve_or_create_user_pk(session, telegram_id)
            if not user_pk:
                logger.warning(f"Skipping log write: could not resolve user pk for telegram_id={telegram_id}")
                return

            log_entry = Log(user_id=user_pk, action=safe_action)
            session.add(log_entry)
            await session.commit()
        except Exception:
            # Best-effort logging: never break bot flow because of logs
            await session.rollback()
            logger.exception("Failed to write Log entry (best-effort)")


async def get_logs(telegram_id: Optional[int] = None, limit: int = 100):
    """
    Get log entries, optionally filtered by Telegram user.
    ...
    async with AsyncSessionLocal() as session:
        ...
        if telegram_id is not None:
            ...
            if not user:
                return []
            query = query.where(Log.user_id == user.id)
