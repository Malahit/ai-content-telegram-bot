...
class TestAlembicEnvConfig(unittest.TestCase):
    ...
    def test_warning_emitted_when_database_url_absent(self):
        """env.py must emit a warning/log call when DATABASE_URL is not set."""
        # Use AST to verify that the else-branch of the DATABASE_URL conditional
        # contains a logging call (warn/warning/error).  This is more resilient
        # than asserting on the exact warning message text, which can be freely
        # rephrased without breaking the safety invariant.
        found_log_in_else = False
        for node in ast.walk(self.tree):
            if isinstance(node, ast.If):
                cond_src = ast.unparse(node.test)
                if ("db_url" in cond_src or "DATABASE_URL" in cond_src) and node.orelse:
                    for child in ast.walk(ast.Module(body=node.orelse, type_ignores=[])):
                        if isinstance(child, ast.Call):
                            func_src = ast.unparse(child.func)
                            if any(kw in func_src for kw in ("warning", "warn", "error")):
                                found_log_in_else = True
                                break
                    break
        self.assertTrue(
            found_log_in_else,
            "alembic/env.py must emit a warning/error log in the else-branch when DATABASE_URL is missing",
        )
