...
class TestMainPyEntrypoint(unittest.TestCase):
    ...
    def test_calls_asyncio_run_main(self):
        ...
        # AST check: find asyncio.run(main()) call anywhere in the module.
        # This is more resilient to whitespace / formatting changes than a
        # verbatim string search.
        found = False
        for node in ast.walk(self.tree):
            if (
                ...
                found = True
                break
        self.assertTrue(found, "main.py must call asyncio.run(main()) inside the __main__ guard")
...
class TestBotPyStructure(unittest.TestCase):
    ...
    def test_on_startup_logs_health_summary(self):
        """on_startup() must emit a multi-line startup health summary."""
        # Find the on_startup function in the AST and count its logger.info
        # calls.  A proper health summary block emits several info lines
        # (separator + header + one per component).  Counting calls is more
        # resilient than matching exact wording, which may be rephrased without
        # changing the diagnostic intent.
        on_startup_node = next(
            (
                node
        ...
        self.assertIsNotNone(on_startup_node, "bot.py must define on_startup()")
        info_calls = [
            node
            for node in ast.walk(on_startup_node)
        ...
        self.assertGreaterEqual(
            len(info_calls),
            4,
            "on_startup() should emit at least 4 logger.info() calls (startup health summary block)",
        )
