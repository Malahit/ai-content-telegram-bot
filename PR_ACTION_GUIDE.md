...
- [ ] **Удалить смерженную ветку** (если больше не нужна): `git push origin --delete <branch-name>` и `git branch -D <branch-name>`
- [ ] **Проверить связанные issues**: Убедиться, что связанные задачи закрыты или корректно привязаны к PR
- [ ] **Проверить CI на базовой ветке**: Убедиться, что все проверки прошли успешно на main/master после слияния
- [ ] **Решить о необходимости релиза/тега**: Если требуется новая версия, отметить номер версии для обновления
- [ ] **Обновить документацию**: Если изменилось поведение, обновить соответствующие документы
